...
Equations impl__compute_group_element_for_vote {v_G : _} `{ t_Sized v_G} `{ t_Group v_G} (xi : both f_Z) (vote : both 'bool) (g_pow_yi : both v_G) : both v_G :=
  impl__compute_group_element_for_vote xi vote g_pow_yi  :=
    (f_pow g_pow_yi xi) .* (f_g_pow (ifb vote
    then f_field_one
    else f_field_zero)) : both v_G.
Fail Next Obligation.

Equations impl__div {v_G : _} `{ t_Sized v_G} `{ t_Group v_G} (x : both v_G) (y : both v_G) : both v_G :=
  impl__div x y  :=
    x .* (f_group_inv y) : both v_G.
Fail Next Obligation.

Equations impl__compute_g_pow_yi {v_G : _} {n : both uint_size} `{ t_Sized v_G} `{ t_Group v_G} (i : both uint_size) (xis : both (nseq v_G (is_pure (n)))) : both v_G :=
  impl__compute_g_pow_yi i xis  :=
    ...
    letb prod2 := f_product (f_map (impl__iter (xis.a[(Build_t_Range (f_start := i .+ (ret_both (1 : uint_size))) (f_end := n))])) (fun x =>
      f_clone x)) in
    impl__div prod1 prod2 : both v_G.
Fail Next Obligation.

Equations impl__sub {v_G : _} `{ t_Sized v_G} `{ t_Group v_G} (x : both f_Z) (y : both f_Z) : both f_Z :=
  impl__sub x y  :=
    x .+ (f_neg y) : both f_Z.
Fail Next Obligation.

Equations impl__check_commitment {v_G : _} `{ t_Sized v_G} `{ t_Group v_G} (g_pow_xi_yi_vi : both v_G) (commitment : both f_Z) : both 'bool :=
  impl__check_commitment g_pow_xi_yi_vi commitment  :=
    (f_hash (impl__into_vec (unsize (box_new (array_from_list [g_pow_xi_yi_vi]))))) =.? commitment : both 'bool.
Fail Next Obligation.

Equations impl__commit_to {v_G : _} `{ t_Sized v_G} `{ t_Group v_G} (g_pow_xi_yi_vi : both v_G) : both f_Z :=
  impl__commit_to g_pow_xi_yi_vi  :=
...
Definition t_OVN {v_G : _} `{ t_Sized v_G} `{ t_Group v_G} : choice_type :=
  (t_PhantomData v_G).
Equations f_pd {v_G : _} `{ t_Sized v_G} `{ t_Group v_G} (s : both t_OVN) : both (t_PhantomData v_G) :=
  f_pd s  :=
    bind_both s (fun x =>
      ret_both (x : (t_PhantomData v_G))) : both (t_PhantomData v_G).
Fail Next Obligation.
Equations Build_t_OVN {v_G : _} `{ t_Sized v_G} `{ t_Group v_G} {f_pd : both (t_PhantomData v_G)} : both (t_OVN) :=
  Build_t_OVN  :=
    bind_both f_pd (fun f_pd =>
      ret_both ((f_pd) : (t_OVN))) : both (t_OVN).
Fail Next Obligation.
Notation "'Build_t_OVN' '[' x ']' '(' 'f_pd' ':=' y ')'" := (Build_t_OVN (f_pd := y)).
...
Equations impl__zkp_one_out_of_two {v_G : _} `{ t_Sized v_G} `{ t_Group v_G} (random_w : both f_Z) (random_r : both f_Z) (random_d : both f_Z) (h : both v_G) (xi : both f_Z) (vi : both 'bool) : both (t_OrZKPCommit v_G) :=
  impl__zkp_one_out_of_two random_w random_r random_d h xi vi  :=
    letb w := random_w in
    ifb vi
    ...
    letb c := f_hash (impl__into_vec (unsize (box_new (array_from_list [x;
      ...
      a2;
      b2])))) in
    letb d2 := impl__sub c d1 in
    letb r2 := impl__sub w (xi .* d2) in
    ...
    letb b2 := (f_pow h r2) .* (f_pow (impl__div y f_g) d2) in
    letb c := f_hash (impl__into_vec (unsize (box_new (array_from_list [x;
      y;
      a1;
      b1;
      a2;
      b2])))) in
    letb d1 := impl__sub c d2 in
    letb r1 := impl__sub w (xi .* d1) in
...
Equations impl__zkp_one_out_of_two_validate {v_G : _} `{ t_Sized v_G} `{ t_Group v_G} (h : both v_G) (zkp : both (t_OrZKPCommit v_G)) : both 'bool :=
  impl__zkp_one_out_of_two_validate h zkp  :=
...
Equations impl__schnorr_zkp {v_G : _} `{ t_Sized v_G} `{ t_Group v_G} (random : both f_Z) (h : both v_G) (x : both f_Z) : both (t_SchnorrZKPCommit v_G) :=
  impl__schnorr_zkp random h x  :=
    ...
    letb c := f_hash (impl__into_vec (unsize (box_new (array_from_list [f_g;
      ...
      u])))) in
    letb z := r .+ (c .* x) in
    letm[choice_typeMonad.result_bind_code (t_SchnorrZKPCommit v_G)] hoist2 := ControlFlow_Break (Build_t_SchnorrZKPCommit (f_schnorr_zkp_u := u) (f_schnorr_zkp_c := c) (f_schnorr_zkp_z := z)) in
    ControlFlow_Continue (never_to_any hoist2)) : both (t_SchnorrZKPCommit v_G).
Fail Next Obligation.

Equations impl__schnorr_zkp_validate {v_G : _} `{ t_Sized v_G} `{ t_Group v_G} (h : both v_G) (pi : both (t_SchnorrZKPCommit v_G)) : both 'bool :=
  impl__schnorr_zkp_validate h pi  :=
...
Equations impl__cast_vote {v_G : _} {n : both uint_size} {v_A : _} {impl_574521470_ : _} `{ t_Sized v_G} `{ t_Group v_G} `{ t_Sized v_A} `{ t_Sized impl_574521470_} `{ t_HasActions v_A} `{ t_HasReceiveContext impl_574521470_ 'unit} (ctx : both impl_574521470_) (state : both (t_OvnContractState v_G (both uint_size))) : both (t_Result (v_A × t_OvnContractState v_G (both uint_size)) t_ParseError) :=
  impl__cast_vote ctx state  :=
    ...
    Result_Ok (letb g_pow_yi := impl__compute_g_pow_yi (cast_int (WS2 := _) (f_cvp_i params)) (f_g_pow_xis state) in
    letb g_pow_xi_yi_vi := impl__compute_group_element_for_vote (f_cvp_xi params) (f_cvp_vote params) g_pow_yi in
    letb zkp_vi := impl__zkp_one_out_of_two (f_cvp_zkp_random_w params) (f_cvp_zkp_random_r params) (f_cvp_zkp_random_d params) g_pow_yi (f_cvp_xi params) (f_cvp_vote params) in
...
Equations impl__commit_to_vote {v_G : _} {n : both uint_size} {v_A : _} {impl_574521470_ : _} `{ t_Sized v_G} `{ t_Group v_G} `{ t_Sized v_A} `{ t_Sized impl_574521470_} `{ t_HasActions v_A} `{ t_HasReceiveContext impl_574521470_ 'unit} (ctx : both impl_574521470_) (state : both (t_OvnContractState v_G (both uint_size))) : both (t_Result (v_A × t_OvnContractState v_G (both uint_size)) t_ParseError) :=
  impl__commit_to_vote ctx state  :=
    run (letb '(_,out) := f_get (f_parameter_cursor ctx) in
    letm[choice_typeMonad.result_bind_code t_ParseError] (params : t_CastVoteParam f_Z) := out in
    Result_Ok (letb _ := foldi_both_list (f_into_iter (Build_t_Range (f_start := ret_both (0 : uint_size)) (f_end := n))) (fun i =>
      ssp (fun _ =>
        ifb orb (not (impl__schnorr_zkp_validate ((f_g_pow_xis state).a[i]) ((f_zkp_xis state).a[i]))) (not ((f_round1 state).a[i]))
        then letm[choice_typeMonad.result_bind_code (t_Result (v_A × t_OvnContractState v_G (both uint_size)) t_ParseError)] hoist30 := ControlFlow_Break (Result_Err ParseError) in
    ...
    letb g_pow_yi := impl__compute_g_pow_yi (cast_int (WS2 := _) (f_cvp_i params)) (f_g_pow_xis state) in
    letb g_pow_xi_yi_vi := impl__compute_group_element_for_vote (f_cvp_xi params) (f_cvp_vote params) g_pow_yi in
    letb commit_vi := impl__commit_to g_pow_xi_yi_vi in
...
Equations impl__init_ovn_contract {v_G : _} {n : both uint_size} `{ t_Sized v_G} `{ t_Group v_G} (_ : both 'unit) : both (t_Result (t_OvnContractState v_G (both uint_size)) t_Reject) :=
  impl__init_ovn_contract _  :=
...
Equations impl__register_vote {v_G : _} {n : both uint_size} {v_A : _} {impl_574521470_ : _} `{ t_Sized v_G} `{ t_Group v_G} `{ t_Sized v_A} `{ t_Sized impl_574521470_} `{ t_HasActions v_A} `{ t_HasReceiveContext impl_574521470_ 'unit} (ctx : both impl_574521470_) (state : both (t_OvnContractState v_G (both uint_size))) : both (t_Result (v_A × t_OvnContractState v_G (both uint_size)) t_ParseError) :=
  impl__register_vote ctx state  :=
    run (letb '(_,out) := f_get (f_parameter_cursor ctx) in
    ...
    letb zkp_xi := impl__schnorr_zkp (f_rp_zkp_random params) g_pow_xi (f_rp_xi params) in
...
Equations impl__tally_votes {v_G : _} {n : both uint_size} {v_A : _} {impl_574521470_ : _} `{ t_Sized v_G} `{ t_Group v_G} `{ t_Sized v_A} `{ t_Sized impl_574521470_} `{ t_HasActions v_A} `{ t_HasReceiveContext impl_574521470_ 'unit} (_ : both impl_574521470_) (state : both (t_OvnContractState v_G (both uint_size))) : both (t_Result (v_A × t_OvnContractState v_G (both uint_size)) t_ParseError) :=
  impl__tally_votes _ state  :=
    letb _ := foldi_both_list (f_into_iter (Build_t_Range (f_start := ret_both (0 : uint_size)) (f_end := n))) (fun i =>
      ssp (fun _ =>
        letb g_pow_yi := impl__compute_g_pow_yi i (f_g_pow_xis state) in
        letm[choice_typeMonad.result_bind_code (t_Result (v_A × t_OvnContractState v_G (both uint_size)) t_ParseError)] _ := ControlFlow_Continue (ifb not (impl__zkp_one_out_of_two_validate g_pow_yi ((f_zkp_vis state).a[i]))
        then letm[choice_typeMonad.result_bind_code (t_Result (v_A × t_OvnContractState v_G (both uint_size)) t_ParseError)] hoist31 := ControlFlow_Break (Result_Err ParseError) in
        ControlFlow_Continue (never_to_any hoist31)
        else ControlFlow_Continue (ret_both (tt : 'unit))) in
        ifb not (impl__check_commitment ((f_g_pow_xi_yi_vis state).a[i]) ((f_commit_vis state).a[i]))
        then letm[choice_typeMonad.result_bind_code (t_Result (v_A × t_OvnContractState v_G (both uint_size)) t_ParseError)] hoist32 := ControlFlow_Break (Result_Err ParseError) in
