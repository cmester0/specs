...
From Hacspec Require Import ChoiceEquality Hacspec_Lib Hacspec_Lib_Pre Hacspec_Lib_Comparable.
Require Import Hacspec_aes_jazz.
...
(*** Helper definitions *)
  ...
  (* Lemma det_key_combine {L1 L2 L3 I1 I2 I3} rcon rkey temp2 : deterministic (is_state (key_combine (L1 := L1) (L2 := L2) (L3 := L3) (I1 := I1) (I2 := I2) (I3 := I3) rcon rkey temp2)). *)
  (* Proof. *)
  (*   unfold key_combine. *)
  (*   unfold Hacspec_aes_jazz.key_combine_obligation_1. *)
  (*   unfold Hacspec_aes_jazz.key_combine_obligation_2. *)
  (*   repeat (apply deterministic_put || (apply deterministic_get ; intros) || apply deterministic_ret). *)
  (* Defined. *)
  ...
  Lemma rebuild_128_eq :
    forall {L1 L2 L3 L4 I1 I2 I3 I4},
    forall (v0 v1 v2 v3 : both _ _ 'word U32) ,
      make_vec _ [is_pure v0 ; is_pure v1 ; is_pure v2 ; is_pure v3] = is_pure (rebuild_u128 (L1 := L1) (L2 := L2)  (L3 := L3) (L4 := L4) (I1 := I1) (I2 := I2)  (I3 := I3) (I4 := I4) v0 v1 v2 v3).
    intros.
    (* simpl. *) rewrite rebuild_u128_equation_1.
    (* unfold "shift_left". *) rewrite !lift2_both_equation_1.
    unfold Hacspec_Lib_Pre.shift_left_.
    unfold is_pure.
    (* unfold ".|". *) rewrite !int_or_equation_1.
    unfold Hacspec_Lib_Pre.int_or.
    (* unfold lift3_both ; simpl. *) rewrite !lift2_both_equation_1. unfold lift_both. simpl.
  ...
  Lemma rebuild_32_eq :
    forall {L1 L2 L3 L4 I1 I2 I3 I4},
    forall (v0 v1 v2 v3 : both _ _ 'word U8) ,
      make_vec _ [is_pure v0 ; is_pure v1 ; is_pure v2 ; is_pure v3] = is_pure (rebuild_u32 (L1 := L1) (L2 := L2)  (L3 := L3) (L4 := L4) (I1 := I1) (I2 := I2)  (I3 := I3) (I4 := I4) v0 v1 v2 v3).
  Proof.
    intros.
    (* simpl. *) rewrite rebuild_u32_equation_1.
    (* unfold "shift_left". *) rewrite !lift2_both_equation_1.
    unfold Hacspec_Lib_Pre.shift_left_.
    unfold is_pure.
    (* unfold ".|". *) rewrite !int_or_equation_1.
  ...
  Lemma index_32_eq :
    forall {L1 L2 I1 I2},
      forall (v : both _ _ ('word U128)) (i : nat),
      i < 4 ->
      word.subword (i * U32) U32 (is_pure v) = is_pure (index_u32 (L1 := L1) (L2 := L2) (I1 := I1) (I2 := I2) v (ret_both (repr U32 (Z.of_nat i)))).
  Proof.
    intros.
    unfold word.subword.
    (* unfold index_u32. *) rewrite index_u32_equation_1.
    simpl.
    rewrite !lift2_both_equation_1.
    unfold (* "shift_left", *) Hacspec_Lib_Pre.shift_left_.
    unfold (* "shift_right", *) Hacspec_Lib_Pre.shift_right_.
    rewrite !int_mod_equation_1.
    unfold (* ".%", *) Hacspec_Lib_Pre.int_mod.
    unfold Hacspec_lib.mul. unfold Hacspec_lib.int_mul_inst. rewrite !int_mul_equation_1.
  ...
  Lemma index_8_eq :
    forall {L1 L2 I1 I2},
    forall (v : both _ _ ('word U32)) (i : nat),
      i < 4 ->
      word.subword (i * U8) U8 (is_pure v) = is_pure (index_u8 (L1 := L1) (L2 := L2) (I1 := I1) (I2 := I2) v (ret_both (repr _ (Z.of_nat i)))).
  Proof.
    intros.
    unfold word.subword.
    (* unfold index_u8. *) rewrite index_u8_equation_1.
    simpl.
    rewrite !lift2_both_equation_1.
    unfold (* "shift_left", *) Hacspec_Lib_Pre.shift_left_.
    unfold (* "shift_right", *) Hacspec_Lib_Pre.shift_right_.
  ...
  Lemma is_pure_lift_both_cancel :
    forall {L1 L2 I1 I2 A} (b : both L1 I1 A) {fsubset_loc fsubset_opsig}, is_pure (lift_both (L2 := L2) (I2 := I2) b (fsubset_loc := fsubset_loc) (fsubset_opsig := fsubset_opsig)) = is_pure b.
  Proof.
      intros.
      destruct b.
      simpl.
  ...
  Lemma wpshufd1_eq :
    forall L1 L2 L3 I1 I2 I3,
    forall (rkey : both _ _ ('word U128))  (i : nat) (n : both _ _ ('word U8)),
      i < 4 ->
      wpshufd1 (is_pure rkey) (is_pure n) i =
        is_pure (vpshufd1 (L1 := L1) (L2 := L2) (L3 := L3) (I1 := I1) (I2 := I2) (I3 := I3) rkey n (ret_both (Hacspec_Lib_Pre.repr _ i))).
  Proof.
    Opaque Z.mul.
    clear.
    intros.
    (* unfold vpshufd1. *) rewrite vpshufd1_equation_1.
    unfold wpshufd1.

    Opaque index_u32.
    rewrite is_pure_lift_both_cancel. 
    ...
    unfold Hacspec_lib.mul. unfold Hacspec_lib.int_mul_inst. rewrite !int_mul_equation_1.
    unfold Hacspec_Lib_Pre.int_mul.
    rewrite !lift2_both_equation_1. unfold lift_both. simpl.
    unfold mul_word.
    rewrite cast_int_equation_1. rewrite lift1_both_equation_1. simpl.
    ...
    change (Hacspec_lib.int_xO
                   (Hacspec_lib.int_xO
                      (Hacspec_lib.int_xO
                         (Hacspec_lib.int_xO (Hacspec_lib.int_xO one))))) with (mkword U32 32%Z).
    rewrite !mkwordK.
    ...
    rewrite (Zmod_small _ (modulus 2)).
    f_equal.
    f_equal.
    f_equal.
    all: try now do 4 (destruct i as [ | i ] ; [ easy | ]).
